...
function estate_facebook_login($get_vars){
    ...
    try {
        ...
        $response = $fb->get('/me?fields=id,email,name,first_name,last_name,gender,locale,timezone', $accessToken);
    } catch(Facebook\Exceptions\FacebookResponseException $e) {
        echo 'Graph returned an error: ' . $e->getMessage();
        exit;
    } catch(Facebook\Exceptions\FacebookSDKException $e) {
...
function estate_google_oauth_login($get_vars){
    ...
    {     
       ...
       // $profile_image_url                = filter_var($user['picture'], FILTER_VALIDATE_URL);
		...
		$google_flag = 1;
        wpestate_register_user_via_google($email,$full_name,$user_id,$google_flag); 
